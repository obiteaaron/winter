...
7. WinterRPC 的实现，不是非常注重性能，主要是在中小型系统中使用，性能并不会成为平静瓶颈。
...
8. DUBBO 的序列化实现更抽象化一些，它完全设计了单独的序列化和反序列化对象，可以将多个对象分别写入，所以可以先写基本信息（含调用方法信息），再写参数对象，两个可以独立写，也就可以独立读，但不易直接调用。Winter Rpc为了能够直接用JSON curl调用，并且不依赖注解，采用了内置的反射方式进行JSON序列化。但还是建议生产环境使用Hessian，性能更好且序列化和反序列化的稳定性更好。
